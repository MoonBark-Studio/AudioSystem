...
description: "Review or implement MoonBark Idle plugin integrations. Use when tracing WorldTime or GridPlacement wiring, auditing GodotSignalBus migration, wiring bootstrap-owned SignalBusProvider buses, enforcing root-owned GameProvider context plus subtree fanout, replacing TimeContextNode with WorldTimeProvider plus TimeSignalBus, or checking plugin Core versus Godot boundary violations in the projects workspace."
user-invocable: true
---
...
- In MoonBark Idle, `SimulationBootstrap` is the wiring point. It creates a bootstrap-owned `SignalBusProvider` as a plain service object, initializes typed plugin signal buses from the game-owned runtime, and injects those buses directly into HUD, controllers, and panels.
- Do not add scene nodes solely to store signal buses or rebuild singleton-style service lookup paths. Wiring lives in bootstrap-owned fields, not the scene tree.
- `GameProvider` is a root-owned resource context. Scene roots export the `.tres`, root-level owners consume it, and subtree owners distribute narrow typed dependencies downward.
...
- Time flow today:
  ...
  - `TimeEventBus`
  - `TimeSignalBus`
- The current light update is bootstrap-mediated, not self-subscribed in the node: `SimulationBootstrap` subscribes to `TimeSignalBus.TimeOfDayChangedSignal` and calls `TimeOfDayLightNode.UpdateFromHour(...)`.
- `SignalBusProvider` is bootstrap-owned state, not a `Node`. It holds `TimeSignalBus` and `PlacementSignalBus` for explicit injection into consumers such as `HudManager`, `IdleHotbar`, `PlacementPanel`, and `PlacementController`.
...
- Does the game inject plugin buses through bootstrap or providers instead of reaching through singletons?
...
- `games/moonbark-idle/godot/SimulationBootstrap.cs`
- `games/moonbark-idle/godot/Context/SignalBusProvider.cs`
